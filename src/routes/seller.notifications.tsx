...
export const Route = createFileRoute("/seller/notifications")({ component: SellerNotifs });

function SellerNotifs() {
  ...
  return (
    <PhoneFrame>
      ...
      <PageBody>
        ...
        {orders.length === 0 ? (
          <p className="text-sm text-muted-foreground">No new orders yet.</p>
        ) : (
            ...
            {orders.map(o => (
              <div key={o.id} className="flex gap-3 p-4 rounded-xl bg-card border border-border">
                ...
                <div className="flex-1">
                  <p className="text-sm">New order from <span className="font-medium">{o.buyerName}</span> — {o.fileName}</p>
